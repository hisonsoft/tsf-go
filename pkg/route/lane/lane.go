...
func (l *Lane) Select(ctx context.Context, svc naming.Service, nodes []naming.Instance) []naming.Instance {
	...
	if !ok {
		l.mu.RUnlock()
		log.DefaultLog.Errorw("msg", "[lane.Select] no lane info found in allLanes!", "laneID", laneID)
		return nodes
	}
...
func (l *Lane) selectNormal(ctx context.Context, svc naming.Service, nodes []naming.Instance) []naming.Instance {
	...
	if len(color) > 0 {
		log.DefaultLog.WithContext(ctx).Debugw("msg", "lane take effect, filter color instance!", "color_nodes", color)
	}
	return normal
}
...
func (l *Lane) refreshAllRule() {
	for {
		...
		if err != nil {
			...
			log.DefaultLog.Errorw("msg", "watch lane config failed!", "err", err)
			continue
		}
		...
		for _, spec := range specs {
			...
			if err != nil {
				log.DefaultLog.Errorw("msg", "unmarshal lane rule config failed!", "err", err, "raw", string(spec.Data.Raw()))
				continue
			}
...
func (l *Lane) refreshAllLane() {
	for {
		...
		if err != nil {
			if errors.IsGatewayTimeout(err) || errors.IsClientClosed(err) {
				log.DefaultLog.Errorf("watch lane config deadline or clsoe!exit now!err:=%v", err)
				return
			}
			log.DefaultLog.Errorf("watch lane config failed!err:=%v", err)
		...
		for _, spec := range specs {
			...
			if err != nil {
				log.DefaultLog.Errorw("msg", "unmarshal lane config failed!", "error", err, "raw", spec.Data.Raw())
				time.Sleep(time.Second)
				continue
			}
